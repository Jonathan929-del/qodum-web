...
// Main Function
const PrintButton = ({jobs}:any) => {


    // Jobs array
    const jobsArray = jobs.map((r:any) => {
        return([
            {value:jobs.indexOf(r) + 1},
            {value:moment(r.last_date_of_submission).format('D-MMM-yy')}
        ]);
    });
    ...
    const data:any = [
        {
            columns:[
                {title:'Jobs List', width:{wpx:200}, style:{font:{bold:true, sz:'12', color:{rgb:'ffffff'}}, fill:{patternType:'solid', fgColor:{rgb:'16365C'}}}},
        ...
        {
            columns: [
                {title:'Sr. No.', width:{wpx:75}, style:{font:{bold:true}, fill:{patternType:'solid', fgColor:{rgb:'C9CACC'}}}},
                {title:'Last Date of Submission', width:{wpx:125}, style:{font:{bold:true}, fill:{patternType:'solid', fgColor:{rgb:'C9CACC'}}}}
            ],
            data:jobsArray,
        }
    ];
