...
// Main Function
const ViewCom = ({setIsViewOpened, designations, setUpdateDesignation}:any) => {
    ...
    const selectHandler = (r:any) => {
        setUpdateDesignation({
            ...
            show_in_payroll:r.show_in_payroll
        });
                    ...
                    <CommandList>
                        {
                            designations.length < 1 ? (
                                <p className='w-full flex flex-row p-2 text-sm bg-[#E2E4FF] border-b-[0.5px] border-[#ccc]'>
                                    ...
                                    <LoadingIcon />
                                ) : designations.map((c:any) => (
                                    <CommandItem
                                        value={`${designations.indexOf(c) + 1} ${c.designation} ${c.show_in_payroll ? 'True' : 'False'}`}
                                        className='w-full min-w-[300px] flex flex-row text-[10px] bg-[#E2E4FF] border-b-[0.5px] border-[#ccc] sm:text-xs md:text-md'
                                    >
                                        <li className='basis-[20%] flex flex-row items-center px-2 border-r-[0.5px] border-[#ccc]'>{designations.indexOf(c) + 1}</li>
                                        ...
                                        <li className='basis-[30%] flex flex-row items-center px-2 border-r-[0.5px] border-[#ccc]'>{c.designation}</li>
                                        <li className='basis-[30%] flex flex-row items-center px-2'>{c.show_in_payroll ? 'True' : 'False'}</li>
                                    </CommandItem>
                                ))
                        }
                    </CommandList>
