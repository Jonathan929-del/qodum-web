...
// Main Function
const ViewCom = ({setIsViewOpened, staffTypes, setUpdateStaffType}:any) => {
    ...
    const selectHandler = (r:any) => {
        setUpdateStaffType({
            ...
            is_hourly_paid:r.is_hourly_paid,
            show_on_ecare:r.show_on_ecare
        });
                    ...
                    <CommandList>
                        {
                            staffTypes.length < 1 ? (
                                <p className='w-full flex flex-row p-2 text-sm bg-[#E2E4FF] border-b-[0.5px] border-[#ccc]'>
                                    ...
                                    <LoadingIcon />
                                ) : staffTypes.map((c:any) => (
                                    <CommandItem
                                        value={`${staffTypes.indexOf(c) + 1} ${c.staff_type} ${c.is_hourly_paid ? 'True' : 'False'} ${c.show_on_ecare ? 'True' : 'False'}`}
                                        className='w-full min-w-[300px] flex flex-row text-[10px] bg-[#E2E4FF] border-b-[0.5px] border-[#ccc] sm:text-xs md:text-md'
                                    >
                                        <li className='basis-[20%] flex flex-row items-center px-2 border-r-[0.5px] border-[#ccc]'>{staffTypes.indexOf(c) + 1}</li>
                                        ...
                                        <li className='basis-[30%] flex flex-row items-center px-2 border-r-[0.5px] border-[#ccc]'>{c.staff_type}</li>
                                        <li className='basis-[15%] flex flex-row items-center px-2 border-r-[0.5px] border-[#ccc]'>{c.is_hourly_paid ? 'True' : 'False'}</li>
                                        <li className='basis-[15%] flex flex-row items-center px-2'>{c.show_on_ecare ? 'True' : 'False'}</li>
                                    </CommandItem>
                                ))
                        }
                    </CommandList>
