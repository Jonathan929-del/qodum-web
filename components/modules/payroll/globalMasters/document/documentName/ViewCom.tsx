...
// Main Function
const ViewCom = ({setIsViewOpened, documents, setUpdateDocument}:any) => {


    // Select handler
    const selectHandler = (document:any) => {
        setUpdateDocument({
            id:document._id,
                    ...
                    <CommandList>
                        {
                            documents.length < 1 ? (
                                <p className='w-full flex flex-row p-2 text-sm bg-[#E2E4FF] border-b-[0.5px] border-[#ccc]'>
                                    ...
                                    <CommandItem
                                        key={index}
                                        value={`${documents.indexOf(document) + 1} ${document.document_type} ${document.document_name} ${moment(document.updateAt).format('D-MMM-yy')}`}
                                        className='w-full min-w-[700px] flex flex-row text-[10px] bg-[#E2E4FF] border-b-[0.5px] border-[#ccc] sm:text-xs md:text-md'
                                    >
                                        <li className='basis-[10%] flex flex-row items-center px-2 border-r-[0.5px] border-[#ccc]'>{documents.indexOf(document) + 1}</li>
                                        <li className='basis-[15%] flex flex-row items-center justify-center px-2 border-r-[0.5px] border-[#ccc]'>
                                            <Button
                                                ...
                                                onClick={() => selectHandler(document)}
                                            >
                                                Select
                                            </Button>
                                        </li>
                                        <li className='basis-[25%] flex flex-row items-center px-2 border-r-[0.5px] border-[#ccc]'>{document.document_type}</li>
                                        <li className='basis-[30%] flex flex-row items-center px-2 border-r-[0.5px] border-[#ccc]'>{document.document_name}</li>
                                        <li className='basis-[20%] flex flex-row items-center px-2'>{moment(document.updateAt).format('D-MMM-yy')}</li>
                                    </CommandItem>
                                ))
                        }
                    </CommandList>
                    {documents.length > 1 && <CommandEmpty>No results found</CommandEmpty>}
                </div>
