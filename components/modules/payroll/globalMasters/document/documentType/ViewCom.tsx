...
// Main Function
const ViewCom = ({setIsViewOpened, documentTypes, setUpdateDocumentType}:any) => {
    ...
    return (
        <Command
            className='w-[90%] max-h-[90%] flex flex-col items-center pb-2 gap-2 rounded-[8px] border-[0.5px] border-[#E8E8E8] lg:w-[80%]'
        >
                    ...
                    <CommandList>
                        {
                            documentTypes.length < 1 ? (
                                <p className='w-full min-w-[600px] flex flex-row p-2 text-sm bg-[#E2E4FF] border-b-[0.5px] border-[#ccc]'>
                                    No documentTypes yet
                                </p>
                            ) : !documentTypes[0]?.document_type ? (
                                    <LoadingIcon />
                                ) : documentTypes.map((document:any, index:number) => (
                                    <CommandItem
                                        key={index}
                                        value={`${documentTypes.indexOf(document) + 1} ${document.document_type} ${moment(document.updateAt).format('D-MMM-yy')}`}
                                        className='w-full min-w-[600px] flex flex-row text-[10px] bg-[#E2E4FF] border-b-[0.5px] border-[#ccc] sm:text-xs md:text-md'
                                    >
                                        <li className='basis-[20%] flex flex-row items-center px-2 border-r-[0.5px] border-[#ccc]'>{documentTypes.indexOf(document) + 1}</li>
                                        ...
                                        <li className='basis-[30%] flex flex-row items-center px-2 border-r-[0.5px] border-[#ccc]'>{document.document_type}</li>
                                        <li className='basis-[30%] flex flex-row items-center px-2'>{moment(document.updateAt).format('D-MMM-yy')}</li>
                                    </CommandItem>
                                ))
                        }
                    </CommandList>
