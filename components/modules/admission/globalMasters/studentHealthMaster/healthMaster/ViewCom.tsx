...
// Main Function
const ViewCom = ({setIsViewOpened, healthMasters, setUpdateHealthMaster}:any) => {


    // Select handler
    const selectHandler = (hMaster:any) => {
        setUpdateHealthMaster({
            id:hMaster._id,
            isDeleteClicked:false,
            health_parameter: hMaster.health_parameter,
            unit: hMaster.unit
        });
                    ...
                    <CommandList>
                        {
                            healthMasters.length < 1 ? (
                                <p className='w-full flex flex-row p-2 text-sm bg-[#E2E4FF] border-b-2 border-[#ccc]'>
                                    ...
                                    <LoadingIcon />
                                ) : healthMasters.map((hMaster:any, index: number) => (
                                    <CommandItem
                                        key={index}
