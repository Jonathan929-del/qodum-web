...
// Main Function
const Buttons = ({setIsViewOpened, healthUnits, updateHealthUnit, setUpdateHealthUnit, onSubmit, form}:any) => {
    ...
    // Use effect
    useEffect(() => {
        const grantedPermissions = user?.permissions?.find((p:any) => p.name === 'Admission')?.permissions?.find((pp:any) => pp.sub_menu === 'Health Unit Master');
    ...
    return (
        <div className='flex flex-row items-center justify-center pb-4 mt-10 gap-2 ml-0'>
            {
                updateHealthUnit.id === '' ? permissions.add && (
                    <Button
                        type='submit'
                        ...
                        {permissions.delete && (
                            <AlertDialog>
                                <AlertDialogTrigger
                                    ...
                                    onClick={() => setUpdateHealthUnit({...updateHealthUnit, isDeleteClicked:true})}
                                >
                                    Delete
                                </AlertDialogTrigger>
                                <AlertDialogContent>
                                    ...
                                    <AlertDialogFooter>
                                        <AlertDialogCancel
                                            onClick={() => setUpdateHealthUnit({...updateHealthUnit, isDeleteClicked:false})}
                                        >
                                            No
                                        </AlertDialogCancel>
                                        <Button
            ...
            {/* Print button */}
            {permissions.print && (
                <PrintButton healthUnits={healthUnits}/>
            )}
