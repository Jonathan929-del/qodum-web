...
// Main Function
const PrintButton = ({termMasters}:any) => {


    // term Masters Array
    const termMastersArray = termMasters.map((term:any) => {
        return([
            {value:term.name},
            {value:moment(term.createdAt).format('D-MMM-yy')}
        ]);
    });
    ...
    const data:any = [
        {
            columns:[
                {title:'Term Master List', width:{wpx:400}, style:{font:{bold:true, sz:'20', color:{rgb:'ffffff'}}, fill:{patternType:'solid', fgColor:{rgb:'16365C'}}}},
