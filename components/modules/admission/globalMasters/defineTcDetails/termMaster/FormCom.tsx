...
const FormCom = ({setIsViewOpened, terms, updateTerm, setUpdateTerm}:any) => {
    ...
    // Form
    const form = useForm({
        resolver:zodResolver(TermMasterValidation),
        defaultValues:{
    ...
    // Submit handler
    const onSubmit = async (values:z.infer<typeof TermMasterValidation>) => {
        // Create term
        if(updateTerm.id === ''){
    ...
    return (
        <div className='w-[90%] max-w-[500px] flex flex-col items-center rounded-[8px] border-[0.5px] border-[#E8E8E8] sm:w-[80%]'>
            <h2 className='w-full text-center py-2 text-sm rounded-t-[8px] font-bold bg-[#e7f0f7] text-main-color'>Define Term Master</h2>
            <Form
                {...form}
            >
                <form
