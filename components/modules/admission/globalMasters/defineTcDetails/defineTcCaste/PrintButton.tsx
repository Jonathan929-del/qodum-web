...
const PrintButton = ({casts}:any) => {
    ...
    // casts Array
    const castsArray = casts.map((cast:any) => {
        return([
            {value:cast.name},
            {value:moment(cast.createdAt).format('D-MMM-yy')}
        ]);
    });
    ...
    const data:any = [
        ...
        {
            columns: [
                {title:'Cast', width:{wpx:150}, style:{font:{bold:true}, fill:{patternType:'solid', fgColor:{rgb:'C9CACC'}}}},
