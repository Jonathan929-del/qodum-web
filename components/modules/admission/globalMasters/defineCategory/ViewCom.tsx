...
// Main Function
const ViewCom = ({setIsViewOpened, categories, setUpdateCategory}:any) => {


    // Select handler
    const selectHandler = (cat:any) => {
        setUpdateCategory({
            id:cat._id,
                    ...
                    <CommandList>
                        {
                            categories.length < 1 ? (
                                <p className='w-full flex flex-row p-2 text-sm bg-[#E2E4FF] border-b-2 border-[#ccc]'>
                                    ...
                                    <CommandItem
                                        key={index}
                                        value={`${categories.indexOf(cat) + 1} ${cat.category_name} ${cat.is_default}`}
                                        className='w-full min-w-[600px] flex flex-row text-[10px] bg-[#E2E4FF] border-b-2 border-[#ccc] sm:text-xs md:text-md'
                                    >
                                        <li className='basis-[15%] flex flex-row items-center px-2 border-r-2 border-[#ccc]'>{categories.indexOf(cat) + 1}</li>
                                        <li className='basis-[15%] flex flex-row items-center justify-center px-2 border-r-2 border-[#ccc]'>
                                            <Button
                                                ...
                                                onClick={() => selectHandler(cat)}
                                            >
                                                Select
                                            </Button>
                                        </li>
                                        <li className='basis-[30%] flex flex-row items-center px-2 border-r-2 border-[#ccc]'>{cat.category_name}</li>
                                        <li className='basis-[30%] flex flex-row items-center px-2 border-r-2 border-[#ccc]'>{cat.is_default}</li>
                                        <li className='basis-[30%] flex flex-row items-center px-2'>{moment(cat.updateAt).format('D-MMM-yy')}</li>
                                    </CommandItem>
                                ))
                        }
                    </CommandList>
