...
const Buttons = ({setIsViewOpened, cadetTypes, updateCadetType, setUpdateCadetType, onSubmit, form}:any) => {
    ...
    // Use effect
    useEffect(() => {
        const grantedPermissions = user?.permissions?.find((p:any) => p.name === 'Admission')?.permissions?.find((pp:any) => pp.sub_menu === 'Define Cadet Type');
    ...
    return (
        <div className='flex flex-row items-center justify-center pb-4 mt-10 gap-2 ml-0'>
            {
                updateCadetType.id === '' ? permissions.add && (
                    <Button
                        type='submit'
