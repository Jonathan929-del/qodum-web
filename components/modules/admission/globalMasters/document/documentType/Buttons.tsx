...
// Main Function
const Buttons = ({setIsViewOpened, documentTypes, updateDocumentType, setUpdateDocumentType, onSubmit, form}:any) => {
    ...
    // Use effect
    useEffect(() => {
        const grantedPermissions = user?.permissions?.find((p:any) => p.name === 'Admission')?.permissions?.find((pp:any) => pp.sub_menu === 'Define Document Type');
    ...
    return (
        <div className='flex flex-row items-center justify-center pb-4 mt-10 gap-2 ml-0'>
            {
                updateDocumentType.id === '' ? permissions.add && (
                    <Button
                        type='submit'
                        ...
                        {permissions.delete && (
                            <AlertDialog>
                                <AlertDialogTrigger
                                    ...
                                    onClick={() => setUpdateDocumentType({...updateDocumentType, isDeleteClicked:true})}
                                >
                                    Delete
                                </AlertDialogTrigger>
                                <AlertDialogContent>
                                    ...
                                    <AlertDialogFooter>
                                        <AlertDialogCancel
                                            onClick={() => setUpdateDocumentType({...updateDocumentType, isDeleteClicked:false})}
                                        >
                                            No
                                        </AlertDialogCancel>
                                        <Button
            ...
            {permissions.read_only && (
                <span
                    onClick={() => setIsViewOpened('document-type')}
            ...
            {/* Print button */}
            {permissions.print && (
                <PrintButton documentTypes={documentTypes}/>
            )}
