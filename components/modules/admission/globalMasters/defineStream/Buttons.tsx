...
// Main Function
const Buttons = ({setIsViewOpened, streams, updateStream, setUpdateStream, onSubmit, form}:any) => {
    ...
    // Use effect
    useEffect(() => {
        const grantedPermissions = user?.permissions?.find((p:any) => p.name === 'Admission')?.permissions?.find((pp:any) => pp.sub_menu === 'Define Stream');
    ...
    return (
        <div className='flex flex-row items-center justify-center pb-4 mt-10 gap-2 ml-0'>
            {
                updateStream.id === '' ? permissions.add && (
                    <Button
                        type='submit'
                        ...
                        {permissions.delete && (
                            <AlertDialog>
                                <AlertDialogTrigger
                                    ...
                                    onClick={() => setUpdateStream({...updateStream, isDeleteClicked:true})}
                                >
                                    Delete
                                </AlertDialogTrigger>
                                <AlertDialogContent>
                                    ...
                                    <AlertDialogFooter>
                                        <AlertDialogCancel
                                            onClick={() => setUpdateStream({...updateStream, isDeleteClicked:false})}
                                        >
                                            No
                                        </AlertDialogCancel>
                                        <Button
