...
// Main Function
const ViewCom = ({setIsViewOpened, streams, setUpdateStream}:any) => {


    // Select handler
    const selectHandler = (stream:any) => {
        setUpdateStream({
            id:stream._id,
                    ...
                    <CommandList>
                        {
                            streams.length < 1 ? (
                                <p className='w-full flex flex-row p-2 text-sm bg-[#E2E4FF] border-b-[0.5px] border-[#ccc]'>
                                    ...
                                    <CommandItem
                                        value={`${streams.indexOf(stream) + 1} ${stream.stream_name}`}
                                        className='w-full min-w-[650px] flex flex-row text-[10px] bg-[#E2E4FF] border-b-[0.5px] border-[#ccc] sm:text-xs md:text-md'
                                    >
                                        <li className='basis-[15%] flex flex-row items-center px-2 border-r-[0.5px] border-[#ccc]'>{streams.indexOf(stream) + 1}</li>
                                        <li className='basis-[15%] flex flex-row items-center justify-center px-2 border-r-[0.5px] border-[#ccc]'>
                                            <Button
                                                ...
                                                onClick={() => selectHandler(stream)}
                                            >
                                                Select
                                            </Button>
                                        </li>
                                        <li className='basis-[40%] flex flex-row items-center px-2 border-r-[0.5px] border-[#ccc]'>{stream.stream_name}</li>
                                        <li className='basis-[40%] flex flex-row items-center px-2'>{moment(stream.updateAt).format('D-MMM-yy')}</li>
                                    </CommandItem>
                                ))
                        }
                    </CommandList>
                    {streams.length > 0 && <CommandEmpty>No results found</CommandEmpty>}
                </div>
