...
const Buttons = ({setIsViewOpened, enquiryNoSettings, updateEnquiryNoSetting, setUpdateEnquiryNoSetting, onSubmit, form}:any) => {
    ...
    // Use effect
    useEffect(() => {
        const grantedPermissions = user?.permissions?.find((p:any) => p.name === 'Admission')?.permissions?.find((pp:any) => pp.sub_menu === 'Enquiry No Setting');
    ...
    return (
        <div className='flex flex-row items-center justify-center pb-4 mt-10 gap-2 ml-0'>
            {
                updateEnquiryNoSetting.id === '' ? permissions.add && (
                    <Button
                        type='submit'
