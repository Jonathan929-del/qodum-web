...
// Main Function
const ViewCom = ({setIsViewOpened, enquiryNoSettings, setUpdateEnquiryNoSetting}:any) => {
    ...
    const selectHandler = (s:any) => {
        setUpdateEnquiryNoSetting({
            ...
            session:s.session,
            enquiry_no_setting_should_be:s.enquiry_no_setting_should_be,
            prefix:s.prefix,
            start_from:s.start_from,
            lead_zero:s.lead_zero,
            suffix:s.suffix
        });
    ...
    return (
        ...
        >
            ...
            <div className='flex flex-row items-center justify-between w-full px-2 py-2 text-sm font-bold text-main-color bg-[#e7f0f7] rounded-t-[8px]'>
                <h2>Enquiry No. Settings List</h2>
            ...
            <div className='w-[95%] h-[90%] flex flex-col items-center bg-[#F1F1F1] rounded-[8px]'>
                ...
                <div className='w-full flex flex-col h-[90%] overflow-scroll custom-sidebar-scrollbar'>
                    ...
                    <CommandList>
                        {
                            enquiryNoSettings.length < 1 ? (
                                <p className='w-full flex flex-row p-2 text-sm bg-[#E2E4FF] border-b-[0.5px] border-[#ccc]'>
                                    No enquiry no settings
                                </p>
                            ) : !enquiryNoSettings[0]?.prefix ? (
                                    <LoadingIcon />
                                ) : enquiryNoSettings.map((c:any) => (
                                    <CommandItem
                                        value={`${enquiryNoSettings.indexOf(c) + 1} ${c.prefix}`}
                                        className='w-full min-w-[300px] flex flex-row text-[10px] bg-[#E2E4FF] border-b-[0.5px] border-[#ccc] sm:text-xs md:text-md'
                                    >
                                        <li className='basis-[20%] flex flex-row items-center px-2 border-r-[0.5px] border-[#ccc]'>{enquiryNoSettings.indexOf(c) + 1}</li>
                                        ...
                                        <li className='basis-[60%] flex flex-row items-center px-2'>{c.prefix}</li>
                                    </CommandItem>
                                ))
                        }
                    </CommandList>
                    {enquiryNoSettings.length > 0 && <CommandEmpty>No results found</CommandEmpty>}
                </div>
