...
const Buttons = ({setIsViewOpened, layouts, updateLayout, setUpdateLayout, onSubmit, form}:any) => {
    ...
    // Use effect
    useEffect(() => {
        const grantedPermissions = user?.permissions?.find((p:any) => p.name === 'Admission')?.permissions?.find((pp:any) => pp.sub_menu === 'Report Layout Setting');
    ...
    return (
        <div className='flex flex-row items-center justify-center pb-4 mt-10 gap-2 ml-0'>
            {
                updateLayout.id === '' ? permissions.add && (
                    <Button
                        type='submit'
