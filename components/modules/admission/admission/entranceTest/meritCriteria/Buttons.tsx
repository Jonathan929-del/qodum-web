...
// Main Function
const Buttons = ({setIsViewOpened, meritCriterias, updateMeritCriteria, setUpdateMeritCriteria, onSubmit, form}:any) => {
    ...
    // Use effect
    useEffect(() => {
        const grantedPermissions = user?.permissions?.find((p:any) => p.name === 'Admission')?.permissions?.find((pp:any) => pp.sub_menu === 'Define Merit Criteria');
    ...
    return (
        <div className='flex flex-row items-center justify-center pb-4 mt-10 gap-2 ml-0'>
            {
                updateMeritCriteria.id === '' ? permissions.add && (
                    <Button
                        type='submit'
                        ...
                        {permissions.delete && (
                            <AlertDialog>
                                <AlertDialogTrigger
                                    ...
                                    onClick={() => setUpdateMeritCriteria({...updateMeritCriteria, isDeleteClicked:true})}
                                >
                                    Delete
                                </AlertDialogTrigger>
                                <AlertDialogContent>
                                    ...
                                    <AlertDialogFooter>
                                        <AlertDialogCancel
                                            onClick={() => setUpdateMeritCriteria({...updateMeritCriteria, isDeleteClicked:false})}
                                        >
                                            No
                                        </AlertDialogCancel>
                                        <Button
            ...
            {/* Print button */}
            {permissions.print && (
                <PrintButton meritCriterias={meritCriterias}/>
            )}
