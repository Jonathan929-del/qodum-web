...
// Main Function
const ViewCom = ({setIsViewOpened, meritCriterias, setUpdateMeritCriteria}:any) => {


    // Select handler
    const selectHandler = (c:any) => {
        setUpdateMeritCriteria({
            id:c._id,
            isDeleteClicked:false,
            session:c.session,
            name:c.name,
            maximum_point:c.maximum_point
        });
        ...
        >
            ...
            <div className='w-[95%] h-[90%] flex flex-col items-center bg-[#F1F1F1] rounded-[8px]'>
                ...
                <div className='w-full flex flex-col h-[90%] overflow-scroll custom-sidebar-scrollbar'>
                    ...
                    <CommandList>
                        {
                            meritCriterias.length < 1 ? (
                                <p className='w-full flex flex-row p-2 text-sm bg-[#E2E4FF] border-b-[0.5px] border-[#ccc]'>
                                    No criterias
                                </p>
                            ) : !meritCriterias[0]?.name ? (
                                    <LoadingIcon />
                                ) : meritCriterias.map((m:any, index:number) => (
                                    <CommandItem
                                        key={index}
                                        value={`${meritCriterias.indexOf(m) + 1}`}
                                        className='w-full min-w-[600px] flex flex-row text-[10px] bg-[#E2E4FF] border-b-[0.5px] border-[#ccc] sm:text-xs md:text-md'
                                    >
                                        <li className='basis-[15%] flex flex-row items-center px-2 border-r-[0.5px] border-[#ccc]'>{meritCriterias.indexOf(m) + 1}</li>
                                        ...
                                        <li className='basis-[25%] flex flex-row items-center px-2 border-r-[0.5px] border-[#ccc]'>{m.name}</li>
                                        <li className='basis-[25%] flex flex-row items-center px-2 border-r-[0.5px] border-[#ccc]'>{m.maximum_point}</li>
                                        <li className='basis-[20%] flex flex-row items-center px-2'>{m.session}</li>
                                    </CommandItem>
                                ))
                        }
                    </CommandList>
