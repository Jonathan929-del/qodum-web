...
// Main Function
const ViewCom = ({setIsViewOpened, boards, setUpdateBoard}:any) => {


    // Select handler
    const selectHandler = (board:any) => {
        setUpdateBoard({
            id:board._id,
                    ...
                    <CommandList>
                        {
                            boards.length < 1 ? (
                                <p className='w-full flex flex-row p-2 text-sm bg-[#E2E4FF] border-b-2 border-[#ccc]'>
                                    ...
                                    <CommandItem
                                        value={`${boards.indexOf(board) + 1} ${board.board} ${board.is_default ? 'True' : 'False'}`}
                                        className='w-full min-w-[600px] flex flex-row text-[10px] bg-[#E2E4FF] border-b-2 border-[#ccc] sm:text-xs md:text-md'
                                    >
                                        <li className='basis-[15%] flex flex-row items-center px-2 border-r-2 border-[#ccc]'>{boards.indexOf(board) + 1}</li>
                                        <li className='basis-[15%] flex flex-row items-center justify-center px-2 border-r-2 border-[#ccc]'>
                                            <Button
                                                ...
                                                onClick={() => selectHandler(board)}
                                            >
                                                Select
                                            </Button>
                                        </li>
                                        <li className='basis-[30%] flex flex-row items-center px-2 border-r-2 border-[#ccc]'>{board.board}</li>
                                        <li className='basis-[15%] flex flex-row items-center px-2 border-r-2 border-[#ccc]'>{board.is_default ? 'True' : 'False'}</li>
                                        <li className='basis-[25%] flex flex-row items-center px-2'>{moment(board.updateAt).format('D-MMM-yy')}</li>
                                    </CommandItem>
                                ))
                        }
                    </CommandList>
