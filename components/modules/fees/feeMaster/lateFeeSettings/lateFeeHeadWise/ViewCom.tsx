...
// Main Function
const ViewCom = ({setIsViewOpened, lateFees, setUpdateLateFee}:any) => {


    // Select handler
    const selectHandler = (f: any) => {
        setUpdateLateFee({
            id:f._id,
                    ...
                    <CommandList>
                        {
                            ...
                            ) : !lateFees[0]?.fee_group ? (
                                <LoadingIcon />
                            ) : lateFees.map((f:any, index:number) => (
                                <CommandItem
                                    key={index}
                                    value={`${lateFees.indexOf(f) + 1} ${f.fee_group} ${f.fee_type} ${f.installment} ${f.head} ${f.amount} ${moment(f.due_date).format('D-MMM-yy')} ${f.late_fee_type}`}
                                    className='w-full min-w-[1100px] flex flex-row text-[10px] bg-[#E2E4FF] border-b-[0.5px] border-[#ccc] sm:text-xs md:text-md'
                                >
                                    <li className='basis-[10%] flex flex-row items-center px-2 border-r-[0.5px] border-[#ccc]'>{lateFees.indexOf(f) + 1}</li>
                                    ...
                                    <li className='basis-[10%] flex flex-row items-center px-2 border-r-[0.5px] border-[#ccc]'>{f.fee_group}</li>
                                    <li className='basis-[15%] flex flex-row items-center px-2 border-r-[0.5px] border-[#ccc]'>{f.fee_type}</li>
                                    <li className='basis-[10%] flex flex-row items-center px-2 border-r-[0.5px] border-[#ccc]'>{f.installment}</li>
                                    <li className='basis-[10%] flex flex-row items-center px-2 border-r-[0.5px] border-[#ccc]'>{f.head}</li>
                                    <li className='basis-[10%] flex flex-row items-center px-2 border-r-[0.5px] border-[#ccc]'>{f.amount}</li>
                                    <li className='basis-[10%] flex flex-row items-center px-2 border-r-[0.5px] border-[#ccc]'>{moment(f.due_date).format('D-MMM-yy')}</li>
                                    <li className='basis-[15%] flex flex-row items-center px-2'>{f.late_fee_type}</li>
                                </CommandItem>
                            ))
                        }
                    </CommandList>
                    {lateFees.length > 1 && <CommandEmpty>No results found</CommandEmpty>}
                </div>
