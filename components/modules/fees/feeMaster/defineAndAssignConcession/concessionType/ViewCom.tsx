...
// Main Function
const ViewCom = ({setIsViewOpened, concessionsTypes, setUpdateConcessionType}:any) => {


    // Select handler
    const selectHandler = (concessionType:any) => {
        setUpdateConcessionType({
            id:concessionType._id,
            ...
            <div className='w-[95%] h-[90%] flex flex-col items-center border-[1px] border-[#ccc] bg-[#F1F1F1] rounded-[8px]'>
                ...
                <div className='w-full flex flex-col h-[90%] overflow-scroll custom-sidebar-scrollbar'>
                    ...
                    <ul className='w-full min-w-[600px] flex flex-row text-[10px] border-b-2 border-[#ccc] text-hash-color cursor-pointer sm:text-xs md:text-md'>
                        ...
                        <li className='basis-[50%] flex flex-row items-center justify-between px-2 border-r-2 border-[#ccc]'>
                            Concession Type
                            <ChevronsUpDown size={12} />
                        </li>
                    ...
                    <CommandList>
                        {
                            concessionsTypes?.length < 1 ? (
                                <p className='w-full flex flex-row p-2 text-sm bg-[#E2E4FF] border-b-2 border-[#ccc]'>
                                    No concession Types yet
                                </p>
                            ) : !concessionsTypes[0]?.type ? (
                                <LoadingIcon />
                            ) : concessionsTypes.map((concessionType: any, index: number) => (
                                <CommandItem
                                    key={index}
                                    value={`${concessionsTypes.indexOf(concessionType) + 1} ${concessionType.type} ${moment(concessionType.updateAt).format('D-MMM-yy')}`}
                                    className='w-full min-w-[600px] flex flex-row text-[10px] bg-[#E2E4FF] border-b-2 border-[#ccc] sm:text-xs md:text-md'
                                >
                                    <li className='basis-[15%] flex flex-row items-center px-2 border-r-2 border-[#ccc]'>{concessionsTypes.indexOf(concessionType) + 1}</li>
                                    <li className='basis-[15%] flex flex-row items-center justify-center px-2 border-r-2 border-[#ccc]'>
                                        <Button
                                            ...
                                            onClick={() => selectHandler(concessionType)}
                                        >
                                            Select
                                        </Button>
                                    </li>
                                    <li className='basis-[50%] flex flex-row items-center px-2 border-r-2 border-[#ccc]'>{concessionType.type}</li>
                                    <li className='basis-[20%] flex flex-row items-center px-2'>{moment(concessionType.updateAt).format('D-MMM-yy')}</li>
                                </CommandItem>
                            ))
                        }
                    </CommandList>
