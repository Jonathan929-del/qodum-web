...
const Buttons = ({setIsViewOpened, feeEntrySettings, updateFeeEntrySetting, setUpdateFeeEntrySetting, onSubmit, form}:any) => {
    ...
    // Use effect
    useEffect(() => {
        const grantedPermissions = user?.permissions?.find((p:any) => p.name === 'Fees')?.permissions?.find((pp:any) => pp.sub_menu === 'Fee Entry Setting');
    ...
    return (
        <div className='flex flex-row items-center justify-center pb-4 mt-10 gap-2 ml-0'>
            {
                updateFeeEntrySetting.id === '' ? permissions.add && (
                    <Button
                        type='submit'
