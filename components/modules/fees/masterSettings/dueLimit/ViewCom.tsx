...
// Main Function
const ViewCom = ({setIsViewOpened, dueLimits, setUpdateDueLimit}:any) => {
    ...
    const selectHandler = (d:any) => {
        setUpdateDueLimit({
            ...
            fee_type:d.fee_type,
            late_fee_on_due:d.late_fee_on_due,
            dues_amount:d.dues_amount,
            is_percent:d.is_percent,
            heads:d.heads,
            fine_waive_off_setting:d.fine_waive_off_setting
        });
                    ...
                    <CommandList>
                        {
                            ...
                            ) : !dueLimits[0]?.class_name ? (
                                    <LoadingIcon />
                                ) : dueLimits.map((d:any) => (
                                    <CommandItem
                                        value={`${dueLimits.indexOf(d) + 1} ${d.class_name} ${d.fee_type} ${d.dues_amount} ${d.is_percent ? 'True' : 'False'} ${d.heads}`}
                                        className='w-full min-w-[800px] flex flex-row text-[10px] bg-[#E2E4FF] border-b-[0.5px] border-[#ccc] sm:text-xs md:text-md'
                                    >
                                        <li className='basis-[10%] flex flex-row items-center px-2 border-r-[0.5px] border-[#ccc]'>{dueLimits.indexOf(d) + 1}</li>
                                        ...
                                        <li className='basis-[15%] flex flex-row items-center px-2 border-r-[0.5px] border-[#ccc]'>{d.class_name}</li>
                                        <li className='basis-[20%] flex flex-row items-center px-2 border-r-[0.5px] border-[#ccc]'>{d.fee_type}</li>
                                        <li className='basis-[15%] flex flex-row items-center px-2 border-r-[0.5px] border-[#ccc]'>{d.dues_amount}</li>
                                        <li className='basis-[10%] flex flex-row items-center px-2 border-r-[0.5px] border-[#ccc]'>{d.is_percent ? 'True' : 'False'}</li>
                                        <li className='basis-[15%] flex flex-row items-center px-2'>{d.heads}</li>
                                    </CommandItem>
                                ))
                        }
                    </CommandList>
                    {dueLimits.lenght > 1 && <CommandEmpty>No results found</CommandEmpty>}
                </div>
