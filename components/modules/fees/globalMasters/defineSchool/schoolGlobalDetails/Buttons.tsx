...
const Buttons = ({setIsViewOpened, schoolsDetails, updateSchoolDetails, setUpdateSchoolDetails, onSubmit, form, setImageSrc, setFile}:any) => {
    ...
    // Use effect
    useEffect(() => {
        const grantedPermissions = user?.permissions?.find((p:any) => p.name === 'Fees')?.permissions?.find((pp:any) => pp.sub_menu === 'School Global Details');
    ...
    return (
        <div className='flex flex-row items-center justify-center pb-4 mt-2 gap-2 ml-0'>
            {
                updateSchoolDetails.id === '' ? permissions.add && (
                    <Button
                        type='submit'
                        ...
                        {permissions.delete && (
                            <AlertDialog>
                                <AlertDialogTrigger
                                    ...
                                    onClick={() => setUpdateSchoolDetails({...updateSchoolDetails, isDeleteClicked:true})}
                                >
                                    Delete
                                </AlertDialogTrigger>
                                <AlertDialogContent>
                                    ...
                                    <AlertDialogFooter>
                                        <AlertDialogCancel
                                            onClick={() => setUpdateSchoolDetails({...updateSchoolDetails, isDeleteClicked:false})}
                                        >
                                            No
                                        </AlertDialogCancel>
                                        <Button
            ...
            {/* Print button */}
            {permissions.print && (
                <PrintButton schoolsDetails={schoolsDetails}/>
            )}
