...
// Main Function
const ViewCom = ({setIsViewOpened, wings, setUpdateWing}:any) => {


    // Select handler
    const selectHandler = (wing:any) => {
        setUpdateWing({
            id:wing._id,
                    ...
                    <CommandList>
                        {
                            wings.length < 1 ? (
                                <p className='w-full flex flex-row p-2 text-sm bg-[#E2E4FF] border-b-2 border-[#ccc]'>
                                    ...
                                    <CommandItem
                                        value={`${wings.indexOf(wing) + 1} ${wing.wing}`}
                                        className='w-full min-w-[600px] flex flex-row text-[10px] bg-[#E2E4FF] border-b-2 border-[#ccc] sm:text-xs md:text-md'
                                    >
                                        <li className='basis-[15%] flex flex-row items-center px-2 border-r-2 border-[#ccc]'>{wings.indexOf(wing) + 1}</li>
                                        <li className='basis-[15%] flex flex-row items-center justify-center px-2 border-r-2 border-[#ccc]'>
                                            <Button
                                                ...
                                                onClick={() => selectHandler(wing)}
                                            >
                                                Select
                                            </Button>
                                        </li>
                                        <li className='basis-[40%] flex flex-row items-center px-2 border-r-2 border-[#ccc]'>{wing.wing}</li>
                                        <li className='basis-[40%] flex flex-row items-center px-2'>{moment(wing.updateAt).format('D-MMM-yy')}</li>
                                    </CommandItem>
                                ))
                        }
                    </CommandList>
