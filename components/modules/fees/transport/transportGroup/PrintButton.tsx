...
// Main Function
const PrintButton = ({transportGroups}:any) => {


    // Transport groups array
    const excelTransportGroupsArray = transportGroups?.map((transportGroup:any) => {
        return([
            {value:transportGroup.distance_name},
            {value:transportGroup.distance_amount},
            {value:transportGroup.distance_from},
            {value:transportGroup.distance_to},
            {value:moment(transportGroup.createdAt).format('D-MMM-yy')},
        ]);
    });
    ...
    const excelData:any = [
        {
            columns:[
                {title:'Distance Range List', width:{wpx:500}, style:{font:{bold:true, sz:'16', color:{rgb:'ffffff'}}, fill:{patternType:'solid', fgColor:{rgb:'16365C'}}}},
        ...
        {
            columns: [
                {title:'Distance Name', width:{wpx:200}, style:{font:{bold:true}, fill:{patternType:'solid', fgColor:{rgb:'C9CACC'}}}},
                {title:'Amount', width:{wpx:75}, style:{font:{bold:true}, fill:{patternType:'solid', fgColor:{rgb:'C9CACC'}}}},
                {title:'From', width:{wpx:50}, style:{font:{bold:true}, fill:{patternType:'solid', fgColor:{rgb:'C9CACC'}}}},
                {title:'To', width:{wpx:50}, style:{font:{bold:true}, fill:{patternType:'solid', fgColor:{rgb:'C9CACC'}}}},
                {title:'Created Date', width:{wpx:100}, style:{font:{bold:true}, fill:{patternType:'solid', fgColor:{rgb:'C9CACC'}}}}
            ],
            data:excelTransportGroupsArray
        }
    ];
