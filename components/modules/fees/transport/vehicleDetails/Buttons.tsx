...
const Buttons = ({setIsViewOpened, vehiclesDetails, updateVehicleDetails, setUpdateVehicleDetails, onSubmit, form, setServiceDueDate, setInsuranceDueDate}:any) => {
    ...
    // Use effect
    useEffect(() => {
        const grantedPermissions = user?.permissions?.find((p:any) => p.name === 'Fees')?.permissions?.find((pp:any) => pp.sub_menu === 'Define Vehicle Details');
    ...
    return (
        <div className='flex flex-row items-center justify-between pb-4 pt-8 gap-2 ml-0'>
            {
                updateVehicleDetails.id === '' ? permissions.add && (
                    <Button
                        type='submit'
