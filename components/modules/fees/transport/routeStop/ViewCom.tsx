...
// Main Function
const ViewCom = ({setIsViewOpened, routeStops, setUpdateRouteStop}:any) => {
    ...
    const selectHandler = (s:any) => {
        setUpdateRouteStop({
            ...
            morning_arrival_time:{
                hour:s.morning_arrival_time.hour,
                minute:s.morning_arrival_time.minute,
                meridiem:s.morning_arrival_time.meridiem
            },
            afternoon_arrival_time:{
                hour:s.afternoon_arrival_time.hour,
                minute:s.afternoon_arrival_time.minute,
                meridiem:s.afternoon_arrival_time.meridiem
            },
            transport_groups:{
                jan:s.transport_groups.jan,
                feb:s.transport_groups.feb,
                mar:s.transport_groups.mar,
                apr:s.transport_groups.apr,
                may:s.transport_groups.may,
                jun:s.transport_groups.jun,
                jul:s.transport_groups.jul,
                aug:s.transport_groups.aug,
                sep:s.transport_groups.sep,
                oct:s.transport_groups.oct,
                nov:s.transport_groups.nov,
                dec:s.transport_groups.dec
            }
        });
                    ...
                    <CommandList>
                        {
                            ...
                            ): !routeStops[0]?.route_no ? (
                                <LoadingIcon />
                            ) : routeStops.map((s:any) => (
                                <CommandItem
                                    value={`${routeStops.indexOf(s) + 1}`}
                                    className='w-full min-w-[700px] flex flex-row text-[10px] bg-[#E2E4FF] border-b-[0.5px] border-[#ccc] sm:text-xs md:text-md'
                                >
                                    <li className='basis-[10%] flex flex-row items-center px-2 border-r-[0.5px] border-[#ccc]'>{routeStops.indexOf(s) + 1}</li>
                                    ...
                                    <li className='basis-[15%] flex flex-row items-center px-2 border-r-[0.5px] border-[#ccc]'>{s?.route_no}</li>
                                    <li className='basis-[15%] flex flex-row items-center px-2 border-r-[0.5px] border-[#ccc]'>{s?.stop_no}</li>
                                    <li className='basis-[15%] flex flex-row items-center px-2 border-r-[0.5px] border-[#ccc]'>{s?.stop_name}</li>
                                    <li className='basis-[15%] flex flex-row items-center px-2 border-r-[0.5px] border-[#ccc]'>{`${s.morning_arrival_time?.hour}:${s.morning_arrival_time?.minute} ${s.morning_arrival_time?.meridiem}`}</li>
                                    <li className='basis-[15%] flex flex-row items-center px-2'>{`${s.afternoon_arrival_time?.hour}:${s.afternoon_arrival_time?.minute} ${s.afternoon_arrival_time?.meridiem}`}</li>
                                </CommandItem>
                            ))
                        }
                    </CommandList>

                    {routeStops.length > 0 && <CommandEmpty>No results found</CommandEmpty>}
                </div>
