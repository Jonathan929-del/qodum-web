...
// Main Function
const Buttons = ({setIsViewOpened, routeStops, updateRouteStop, setUpdateRouteStop, onSubmit, form}:any) => {
    ...
    // Use effect
    useEffect(() => {
        const grantedPermissions = user?.permissions?.find((p:any) => p.name === 'Fees')?.permissions?.find((pp:any) => pp.sub_menu === 'Define Route Stop');
        setPermissions(grantedPermissions);
    }, [user]);

    return (
        <div className='flex flex-row items-center justify-between gap-2 ml-0'>
            {
                updateRouteStop.id === '' ? permissions.add && (
                    <Button
                        type='submit'
                        ...
                        {permissions.delete && (
                            <AlertDialog>
                                <AlertDialogTrigger
                                    ...
                                    onClick={() => setUpdateRouteStop({...updateRouteStop, isDeleteClicked:true})}
                                >
                                    Delete
                                </AlertDialogTrigger>
                                <AlertDialogContent>
                                    ...
                                    <AlertDialogFooter>
                                        <AlertDialogCancel
                                            onClick={() => setUpdateRouteStop({...updateRouteStop, isDeleteClicked:false})}
                                        >
                                            No
                                        </AlertDialogCancel>
                                        <AlertDialogAction>
            ...
            {/* Print button */}
            {permissions.print && (
                <PrintButton routeStops={routeStops}/>
            )}
