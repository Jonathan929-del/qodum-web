...
const Buttons = ({setIsViewOpened, travelMasters, updateTravelMaster, setUpdateTravelMaster, onSubmit, form}:any) => {
    ...
    // Use effect
    useEffect(() => {
        const grantedPermissions = user?.permissions?.find((p:any) => p.name === 'Fees')?.permissions?.find((pp:any) => pp.sub_menu === 'Travel Agency Master');
    ...
    return (
        <div className='flex flex-row items-center justify-between pb-4 pt-8 gap-2 ml-0'>
            {
                updateTravelMaster.id === '' ? permissions.add && (
                    <Button
                        type='submit'
