...
// Main Function
const ViewCom = ({setIsViewOpened, generalLedgers, setUpdateGeneralLedger}:any) => {
    ...
    const selectHandler = (ledger:any) => {
        setUpdateGeneralLedger({
            ...
            is_cash_book:ledger.is_cash_book,
            is_fixed_asset:ledger.is_fixed_asset,
        ...
        >
            ...
            <div className='w-[95%] h-[90%] flex flex-col items-center bg-[#F1F1F1] rounded-[8px]'>
                ...
                <div className='w-full flex flex-col h-[90%] overflow-scroll custom-sidebar-scrollbar'>
                    {/* Headers */}
                    <ul className='w-full min-w-[900px] flex flex-row text-[10px] border-b-[0.5px] border-[#ccc] text-hash-color cursor-pointer md:text-md'>
                    ...
                    <CommandList>
                        {
                            generalLedgers.length < 1 ? (
                                <p className='w-full min-w-[1200px] flex flex-row p-2 text-sm bg-[#E2E4FF] border-b-[0.5px] border-[#ccc]'>
                                    No general ledgers yet
                                </p>
                            ):
                            !generalLedgers[0]?.account_name ? (
                                <LoadingIcon />
                            ) : generalLedgers.map((ledger:any) => (
                                <CommandItem
                                    value={`${generalLedgers.indexOf(ledger) + 1} ${ledger.account_name} ${ledger.group} ${ledger.opening_balance} ${ledger.opening_balance_type} ${ledger.is_cash_book ? 'Yes' : 'No'}`}
                                    className='w-full min-w-[900px] flex flex-row text-[10px] bg-[#E2E4FF] border-b-[0.5px] border-[#ccc] sm:text-xs md:text-md'
                                >
                                    <li className='basis-[10%] flex flex-row items-center px-2 border-r-[0.5px] border-[#ccc]'>{generalLedgers.indexOf(ledger) + 1}</li>
                                    ...
                                    <li className='basis-[20%] flex flex-row items-center px-2 border-r-[0.5px] border-[#ccc]'>{ledger.account_name}</li>
                                    <li className='basis-[15%] flex flex-row items-center px-2 border-r-[0.5px] border-[#ccc]'>{ledger.group}</li>
                                    <li className='basis-[15%] flex flex-row items-center px-2 border-r-[0.5px] border-[#ccc]'>{ledger.opening_balance}</li>
                                    <li className='basis-[15%] flex flex-row items-center px-2 border-r-[0.5px] border-[#ccc]'>{ledger.opening_balance_type}</li>
                                    <li className='basis-[15%] flex flex-row items-center px-2 border-r-[0.5px] border-[#ccc]'>{ledger.is_cash_book ? 'Yes' : 'No'}</li>
                                </CommandItem>
                            ))
                        }
                    </CommandList>
                    {generalLedgers.length > 0 && <CommandEmpty>No results found</CommandEmpty>}  
                </div>
