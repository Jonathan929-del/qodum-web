...
// Main Function
const ViewCom = ({setIsViewOpened, bankLedgers, setUpdateBankLedger}:any) => {
        ...
        >
            ...
            <div className='w-[95%] h-[90%] flex flex-col items-center bg-[#F1F1F1] rounded-[8px]'>
                ...
                <div className='w-full flex flex-col h-[90%] overflow-scroll custom-sidebar-scrollbar'>
                    ...
                    <CommandList>
                        {
                            ...
                            !bankLedgers[0]?.account_name ? (
                                ...
                                <CommandItem
                                    value={`${bankLedgers.indexOf(ledger) + 1} ${ledger.account_name} ${ledger.group} ${ledger.account_address} ${ledger.account_city} ${ledger.pin_code} ${ledger.email} ${ledger.mobile} ${ledger.opening_balance} ${ledger.opening_balance_type}`}
                                    className='w-full min-w-[1200px] flex flex-row text-[10px] bg-[#E2E4FF] border-b-[0.5px] border-[#ccc] sm:text-xs md:text-md'
                                >
                                    <li className='basis-[5%] flex flex-row items-center px-2 border-r-[0.5px] border-[#ccc]'>{bankLedgers.indexOf(ledger) + 1}</li>
                    ...
                    {bankLedgers.length > 0 && <CommandEmpty>No results found</CommandEmpty>}  
                </div>
