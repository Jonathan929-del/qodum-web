...
const Buttons = ({setIsViewOpened, accountGroups, updateAccountGroup, setUpdateAccountGroup, onSubmit, form}:any) => {
    ...
    // Use effect
    useEffect(() => {
        const grantedPermissions = user?.permissions?.find((p:any) => p.name === 'Accounts')?.permissions?.find((pp:any) => pp.sub_menu === 'Define Account Group');
    ...
    return (
        <div className='flex flex-row items-center justify-between pb-4 pt-8 gap-2 ml-0'>
            {
                updateAccountGroup.id === '' ? permissions.add && (
                    <Button
                        type='submit'
