...
const page = () => {
    ...
    // Is sessions transfered
    const sessionsTransferedFunc = async () => {
        try {
            ...
            const isGroupSessionTransferedRes = await isGroupSesssionTransfered();
            setValues([
                {
                    name:'Define Group',
                    isTransfered:isGroupSessionTransferedRes === 0 ? 'Not Transfered' : 'Already Transfered'
                }
            ]);
    ...
    return (
        <div className='h-full flex flex-row flex-wrap items-start justify-center pt-10 gap-6 bg-white'>
            ...
            {/* Draggables */}
            {showDraggables.includes('Define Group') && (
                <DefineGroup
