...
import {fetchLateFeesHeadWise, lateFeeHeadWiseSesssionTransfer} from '@/lib/actions/fees/feeMaster/lateFeeSettings/lateFeeHeadWise.actions';
...
// Main function
const LateFeeSettingsHeadWise = ({currentSession, nextSession, showDraggables, setShowDraggables, state, setState}:any) => {
    ...
    const transerHandler = async () => {
        try {
            await lateFeeHeadWiseSesssionTransfer({next_session:nextSession});
            setState({...state, isTransfered:true});
            setShowDraggables(showDraggables.filter((d:any) => d !== 'Late Fee Settings Head Wise'));
    ...
    useEffect(() => {
        const fetcher = async () => {
            const lateFeeSettingsRes = await fetchLateFeesHeadWise();
            setLateFees(lateFeeSettingsRes);
        };
        fetcher();
    }, []);
            ...
            >
                ...
                <div className='w-full flex flex-col items-center gap-4'>
                    ...
                    <div className='flex flex-row items-center justify-center w-full px-2 py-2 text-sm font-bold text-main-color bg-[#e7f0f7] rounded-t-[4px]'>
                        <h2>Late Fee Head Wise Settings {currentSession}</h2>
                    </div>
                ...
                <div className='flex flex-row items-center justify-center gap-4'>
                    <AlertDialog>
                        ...
                        <AlertDialogContent>
                            <AlertDialogHeader>
                                <AlertDialogTitle>Are you sure you want to transfer late fee head wise settings from session {currentSession} to {nextSession}</AlertDialogTitle>
                            </AlertDialogHeader>
                            <AlertDialogFooter>
...
// Export
export default LateFeeSettingsHeadWise;