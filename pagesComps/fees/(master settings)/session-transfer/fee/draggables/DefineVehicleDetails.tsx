...
import { fetchVehicleTypes, vehicleTypesSesssionTransfer } from '@/lib/actions/fees/transport/vehicleType.actions';
import { fetchVehiclesDetails, vehiclesDetailsSesssionTransfer } from '@/lib/actions/fees/transport/vehicleDetails.actions';
...
// Main function
const DefineVehicleDetails = ({currentSession, nextSession, showDraggables, setShowDraggables, state, setState}:any) => {
    ...
    const transerHandler = async () => {
        try {
            await vehiclesDetailsSesssionTransfer({next_session:nextSession});
            setState({...state, isTransfered:true});
            setShowDraggables(showDraggables.filter((d:any) => d !== 'Define Vehicle Details'));
    ...
    useEffect(() => {
        const fetcher = async () => {
            const vehicleDetailsRes = await fetchVehiclesDetails();
            setVehicleDetails(vehicleDetailsRes);
        };
        fetcher();
    }, []);
            ...
            >
                ...
                <div className='w-full flex flex-col items-center gap-4'>
                    ...
                    <div className='flex flex-row items-center justify-center w-full px-2 py-2 text-sm font-bold text-main-color bg-[#e7f0f7] rounded-t-[4px]'>
                        <h2>Define Vehicle Details {currentSession}</h2>
                    </div>
                ...
                <div className='flex flex-row items-center justify-center gap-4'>
                    <AlertDialog>
                        ...
                        <AlertDialogContent>
                            <AlertDialogHeader>
                                <AlertDialogTitle>Are you sure you want to transfer vehicle details from session {currentSession} to {nextSession}</AlertDialogTitle>
                            </AlertDialogHeader>
                            <AlertDialogFooter>
...
// Export
export default DefineVehicleDetails;