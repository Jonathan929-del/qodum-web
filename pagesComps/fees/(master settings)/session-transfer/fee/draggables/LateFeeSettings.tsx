...
import {groupsSesssionTransfer, fetchGroups} from '@/lib/actions/fees/feeMaster/feeMaster/group.actions';
import {AlertDialog, AlertDialogCancel, AlertDialogContent, AlertDialogFooter, AlertDialogHeader, AlertDialogTitle, AlertDialogTrigger} from '@/components/ui/alert-dialog';
import { fetchLateFees, lateFeeSesssionTransfer } from '@/lib/actions/fees/feeMaster/lateFeeSettings/lateFee.actions';
...
// Main function
const LateFeeSettings = ({currentSession, nextSession, showDraggables, setShowDraggables, state, setState}:any) => {
    ...
    const transerHandler = async () => {
        try {
            await lateFeeSesssionTransfer({next_session:nextSession});
            setState({...state, isTransfered:true});
            setShowDraggables(showDraggables.filter((d:any) => d !== 'Late Fee Settings'));
    ...
    useEffect(() => {
        const fetcher = async () => {
            const lateFeeSettingsRes = await fetchLateFees();
            ...
            >
                ...
                <div className='w-full flex flex-col items-center gap-4'>
                    ...
                    <div className='flex flex-row items-center justify-center w-full px-2 py-2 text-sm font-bold text-main-color bg-[#e7f0f7] rounded-t-[4px]'>
                        <h2>Late Fee Settings {currentSession}</h2>
                    </div>
                ...
                <div className='flex flex-row items-center justify-center gap-4'>
                    <AlertDialog>
                        ...
                        <AlertDialogContent>
                            <AlertDialogHeader>
                                <AlertDialogTitle>Are you sure you want to transfer late fee settings from session {currentSession} to {nextSession}</AlertDialogTitle>
                            </AlertDialogHeader>
                            <AlertDialogFooter>
