...
import {concessionsSesssionTransfer, fetchConcessions} from '@/lib/actions/fees/feeMaster/defineAndAssignConcession/concession.actions';
import {concessionTypeSesssionTransfer, fetchConcessionsTypes } from '@/lib/actions/fees/feeMaster/defineAndAssignConcession/concessionType.actions';
...
// Main function
const DefineConcession = ({currentSession, nextSession, showDraggables, setShowDraggables, state, setState}:any) => {
    ...
    const transerHandler = async () => {
        try {
            await concessionsSesssionTransfer({next_session:nextSession});
            setState({...state, isTransfered:true});
            setShowDraggables(showDraggables.filter((d:any) => d !== 'Define Concession Type'));
    ...
    // Skip handler
    const skipHandler = () => {
        setShowDraggables(showDraggables.filter((d:any) => d !== 'Define Concession'));
        toast({title:'Process completed successfully'});
    };
    ...
    useEffect(() => {
        const fetcher = async () => {
            const concessionRes = await fetchConcessions();
            setConcessions(concessionRes);
        };
        fetcher();
    }, []);
            ...
            >
                ...
                <div className='w-full flex flex-col items-center gap-4'>
                    ...
                    <div className='flex flex-row items-center justify-center w-full px-2 py-2 text-sm font-bold text-main-color bg-[#e7f0f7] rounded-t-[4px]'>
                        <h2>Define Concession {currentSession}</h2>
                    </div>
                ...
                <div className='flex flex-row items-center justify-center gap-4'>
                    <AlertDialog>
                        ...
                        <AlertDialogContent>
                            <AlertDialogHeader>
                                <AlertDialogTitle>Are you sure you want to transfer concessions from session {currentSession} to {nextSession}</AlertDialogTitle>
                            </AlertDialogHeader>
                            <AlertDialogFooter>
...
// Export
export default DefineConcession;